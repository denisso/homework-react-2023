...
const Title = ({
  ...
  return (
    <div
      ...
      onClick={onClick}
    >
      <div className={text ? styles.text : styles.placeholder}>
        {text === undefined || text === "" ? placeholder : text}
      </div>
      <IconOpenClose className={`${styles.icon} ${active ? styles.active : ""}`} />
    </div>
  );
};

export type DropDownItems = { value: string; text: string };

const DropDownMenu = ({
  onClick,
  items,
}: {
  onClick: (arg: string | null) => void;
  items: DropDownItems[];
}) => {
  return (
    <div className={styles.dropdownMenu}>
      <div onClick={() => onClick("")} className={styles.dropdownMenuItem}>
        Не выбран
      </div>
...
type DropDownProps = {
  ...
  onChangeAndClose: (arg: string) => void;
  value?: string;
};
...
const DropDown = ({
  ...
  const handleClick = () => {
    const newActive = active ? false : true;
    setActive(newActive);
    if (onFocus instanceof Function) onFocus(newActive);
  };
  const handleItemDD = (id: string | null) => {
    setActive(active ? false : true);
    if (onChangeAndClose instanceof Function && typeof id === "string") {
      onChangeAndClose(id);
      setText(items.find((e) => e.value === id)?.text ?? "");
    }
  };
  return (
    <div className={styles.dropdown}>
      <Title
        ...
        active={active}
        text={
          text !== "" ? text : items?.find((e) => e.value === value)?.text ?? ""
        }
      />
